...
export const intradayEn: IntradayContent = {
  ...
  experience: {
    label: "The solutions",
    hint: "Each screen is live. Scroll to read the case; interact only if you want to.",
    heading: "Five solutions",
    lede: "The forecast can be replaced while the planner is looking somewhere else. Five pieces of design make that safe to live with.",
    solutions: [
      {
        id: "allow",
        label: "Allow it, one switch per queue",
        statement: "A single switch on the queue, and no detection settings anywhere.",
        line: "Thresholds, period counts and volume cutoffs stay inside the system. The customer decides which queues may be reforecast, not how detection works.",
        look: "The switch sits under the settings the queue already had.",
        view: { phase: 3, place: "configurations", reforecastOn: true },
      },
      {
        id: "announce",
      ...
      {
        id: "replace",
        label: "Replace the line, and name the issue",
        statement: "When the run finishes the new line is the forecast, and the notice points at what to check.",
        line: "Completion is not the end of the task. The planner still has to decide whether the day is staffed for the new numbers.",
        look: "The state reads Forecast updated, and the banner leads to the issue.",
        view: { phase: 5, place: "forecast" },
      },
      {
        id: "compare",
      ...
      {
        id: "locate",
        label: "Open the exact period",
        statement: "From the issue, one action opens the affected period on the forecast, and another opens it on the schedule.",
        line: "Knowing the forecast changed is not enough to act. The planner needs the hour, and the people already on shift in it.",
        look: "Either button moves you, and keyboard focus, straight to 10:00 to 11:00.",
        view: { phase: 5, place: "issues", nav: true },
      },
    ],
  },
  problem: {
    heading: "The problem",
    lede: "A forecast written this morning stops describing the day by lunchtime. Reforecasting fixes the numbers. It also means the plan can be rewritten while nobody is asking.",
    story:
    ...
    facts: [
      {
        term: "Same day only",
        detail: "The account's day, in the account's time zone.",
      },
      {
        term: "A check every 30 minutes",
        detail: "A check is not a new forecast. Most checks change nothing.",
      },
      {
        term: "Run only on a gap that holds",
        detail: "Real values must pull away from the forecast over several periods, on a queue with enough volume.",
      },
      {
        term: "The new forecast is the forecast",
        detail: "It goes to Forecast, Team schedule, Insights and reporting. The original is kept as a baseline.",
      },
    ],
    undefinedNote:
      "The threshold, the number of periods and the volume cutoff were all left to be defined later. The design had to work without knowing them.",
  },
  cover: {
