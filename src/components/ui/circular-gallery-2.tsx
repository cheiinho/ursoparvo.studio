...
class App {
  ...
  onTouchUp() {
    const tap = this.isDown && this.dragDistance < 12;
    this.isDown = false;
    this.onCheck();
  ...
  onWheel(e: WheelEvent) {
    e.preventDefault();
    const delta =
      Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.deltaY || e.detail;
    this.scroll.target += delta * this.scrollSpeed * 0.12;
  ...
  update() {
    if (this.isVisible) {
      ...
      this.onScrollVelocity?.(velocity);
  ...
  addEventListeners() {
    ...
    this.boundOnWheel = (e: Event) => this.onWheel(e as WheelEvent);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "font-sans text-[24px] font-normal text-[#1a1a1a]",
        fontClassName,
        className,
      )}
      {...props}
